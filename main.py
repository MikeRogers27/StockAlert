...
def send_email(current_price, peak_price, drop_percent):
    ...
    try:
        ...
        new_threshold = drop_percent + 1.0
        state = {'threshold_percent': new_threshold, 'last_updated': datetime.now().isoformat()}
        save_threshold_state(state)
        
        LOG.info(f"Alert sent! S&P 500: {current_price} (down {drop_percent:.1f}% from peak). Next threshold: {new_threshold:.1f}%")
    except Exception as e:
        LOG.error(f"Failed to send email: {e}")

def monitor_sp500_peak_drop(check_interval=300):
